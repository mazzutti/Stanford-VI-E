...
class ValidatorRegistry:
    ...
    def validate_angle_range(cls, angles: Sequence[float] | None) -> None:
        ...
        if invalid:
            raise ValueError(
                f"All angles must be in [0, 90] degrees, got: {invalid}"
            )
    ...
    def validate_non_empty(
        cls, seq: Sequence, name: str = "sequence"
    ) -> None:
    ...
    def validate_length(
        ...
        if len(seq) != expected_len:
            raise ValueError(
                f"{name} must have length {expected_len}, got {len(seq)}"
            )
    ...
    def validate_length_between(
        ...
        if not (min_len <= len(seq) <= max_len):
            raise ValueError(
                f"{name} length must be in [{min_len}, {max_len}], "
                f"got {len(seq)}"
            )
