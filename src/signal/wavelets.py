"""Wavelet helpers moved into src.signal.

Small utility for generating common seismic wavelets.
"""

import numpy as np
import logging

__all__ = ["ricker_wavelet", "Wavelet"]
...
class WaveletHelper:
    def ricker_wavelet(
        ...
        return ricker_wavelet(f_peak=f_peak, length=length, dt=dt)
...
def ricker_wavelet(
    f_peak: float, length: float = 0.128, dt: float = 0.002
) -> np.ndarray:
    """Compatibility wrapper that returns raw numpy samples for callers.

    For users that prefer OOP, instantiate a `Wavelet` with the returned
    samples and dt: `Wavelet(ricker_wavelet(...), dt)`.
    """
